...
- (void)xmppStream:(XMPPStream *)sender didReceiveError:(NSXMLElement *)element
{
	...
	if ([elementName isEqualToString:@"stream:error"] || [elementName isEqualToString:@"error"])
	{
		...
		if (conflict)
		{
            if([SharedAppDelegate.closeFlog isEqualToString:@"login"])
                
            {
                SharedAppDelegate.closeFlog=@"close out";
            }
            else
            {
                UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"Chat service closed unexpectedly" message:@"The user may have logged in on another device." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                [alert show];
            }
