...
**/
- (CGFloat)serverXmppStreamVersionNumber
{
	if (dispatch_get_specific(xmppQueueTag))
	{
...
//    [multicastDelegate xmppStream:self didSendMessage:message];
    ...
    [self xmppStream:self didSendMessage:message];
}
#pragma mark - 发送成功
-(void)xmppStream:(XMPPStream *)sender didSendMessage:(XMPPMessage *)message {
    
    NSLog(@"发送成功内容===%@\n===%@ ",message,[message attributeForName:@"messageid"]);
    NSDate * date = (NSDate *)[message attributeForName:@"date"];
    
//    //本地只保存路径即可
        NSString * messageto =[NSString stringWithFormat:@"%@",message.to];
     NSArray *strs=[messageto componentsSeparatedByString:@"@"];
    NSString * mes=[NSString stringWithFormat:@"%@",message.body];
    
    if (mes.length>4 && [[mes substringToIndex:4]isEqualToString:@"[/9]"]) {
           NSLog(@"视频消息发送成功======%@   接收=%@",mes,messageto);
    }else{
        if (messageto.length>1) {
            //生成消息对象 存储到FMD
//            WCMessageObject *messageObject=[[WCMessageObject alloc] init];
//            messageObject.messageContent=message.body;
//            messageObject.messageFrom=[NSString stringWithFormat:@"%@",message.from];
//            messageObject.messageTo=strs[0];
//            [[XMPPServer sharedServer] sendMessage:messageObject];
           
            // frome  ---- to  addAttributeWithName
            
        }
        
        NSString * mes=[NSString stringWithFormat:@"%@",[message attributeStringValueForName:@"messageid"]];
        [WCMessageObject UpdataMessageSendYesType:mes];
//          [WCMessageObject UpdataMessageSendYesType:strs[0] UserName:[[NSUserDefaults standardUserDefaults] stringForKey:@"userId"]];
        
        
    }
  [[NSNotificationCenter defaultCenter]postNotificationName:@"senmessage" object:nil];
    NSLog(@"消息发送成功==保存数据库=====%@",message.body);
}
#pragma mark - 发送失败
- (void)failToSendMessage:(XMPPMessage *)message error:(NSError *)error
{
    ...
    NSLog(@"zn====发送失败！！");
    if (mes.length>4 && [[mes substringToIndex:4]isEqualToString:@"[/9]"]) {
        NSLog(@"视频消息发送失败======%@   接收=%@",mes,messageto);
    }else{
        
        NSString * mes=[NSString stringWithFormat:@"%@",[message attributeStringValueForName:@"messageid"]];
        [WCMessageObject UpdataMessageSendErrorType:mes];
...
//        [WCMessageObject UpdataMessageState:strs[0] UserName:[[NSUserDefaults standardUserDefaults] stringForKey:@"userId"]];
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"sendmessageerror" object:nil];
    
//     [UIView addMJNotifierWithText:@"Failed to send, check the network" dismissAutomatically:NO];
...
//            [self showMessage:@"Network connection disconnect"];
